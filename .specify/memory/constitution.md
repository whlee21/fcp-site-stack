...
Rationale: MINOR. 새 섹션 "범위 경계"를 추가하고 그 아래 연구자 대상 기능의 범위 배제를
명문화했다. 기존 원칙을 제거하거나 재정의하지 않으며, v4.0.0 아래에서 적법했던 설계가
위헌이 되지도 않는다 — 원칙 II는 이미 데이터 워크스페이스를 통한 연구자 접근을 금지하고
있었고, 이 개정은 "연구자용 분석 워크스페이스를 이 버전에서 만들지 않는다"는 결정을
기록해 그 금지를 유지·강화한다. 따라서 MAJOR가 아니라 MINOR다.

Modified principles: (없음 — 원칙 본문 변경 없음)
- II. 데이터 주권과 PHI 경계 — 새 "범위 경계" 섹션으로의 상호 참조 한 줄만 추가

Added sections:
- 범위 경계 (신규) — 이 솔루션이 다루지 않기로 정한 것을 모으는 섹션.
  첫 항목으로 "연구자 대상 기능(추후 정의)"을 둔다.
...
Downstream artifacts to review under this version (not modified here):
- specs/013-platform-roles/spec.md — 연구자 롤을 카탈로그에 "부여 불가" 상태로 기록한다.
  새 섹션이 "범위 밖이라는 사실과 경계의 기록은 허용(MAY)"을 명시하므로 이 기록은 유효하다.
  다만 그 롤에 실제 권한 항목을 채우는 것은 금지된다.
- specs/009-target-project-job/spec.md — 프로젝트·추출물의 소비자가 연구자인지 운영 측
  인원인지가 확정되지 않았다. 연구자라면 User Story 4가 이 범위 배제에 걸린다. v4.0.0에서
  이미 재검토 대상으로 표시했고, 이 개정으로 재검토 사유가 하나 늘었다.
...
- TODO(RESEARCH_ANALYSIS_WORKSPACE): 연구자용 분석 워크스페이스는 이제 "미정의"가 아니라
  **명시적 범위 밖**이다. 범위 경계 섹션이 그 근거를 담는다. 이 배제를 해제하려면 헌법을
  개정해야 하며, 그때 유형·계층 구성·접근 모델·전달 경로를 함께 정한다.
...
**Version**: 4.1.0 | **Ratified**: 2026-08-28 | **Last Amended**: 2026-09-01
